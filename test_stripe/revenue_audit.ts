 ...
 *
 * Run: deno run --allow-env --allow-net --allow-write test_stripe/revenue_audit.ts
 *
 * Required env vars:
...
const USE_TEST_MODE = Deno.env.get("USE_TEST_MODE") === "true";
const STRIPE_KEY = USE_TEST_MODE
  ? Deno.env.get("STRIPE_TEST_SECRET_KEY")
  : Deno.env.get("STRIPE_SECRET_KEY");

if (!STRIPE_KEY) {
  console.error("❌  No Stripe key found. Set STRIPE_SECRET_KEY or STRIPE_TEST_SECRET_KEY + USE_TEST_MODE=true");
  Deno.exit(1);
}
...
async function stripeGet(path: string, params: Record<string, string> = {}) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Stripe GET ${path} failed: ${err.slice(0, 300)}`);
  }
  return res.json();
}
...
async function getSubscriptionsForCustomer(customerId: string): Promise<any[]> {
...
const report = {
  ...
  past_due: [] as any[],
  canceled: [] as any[],
  no_subscription: [] as any[], // Card on file but ZERO subscription
...
await Deno.writeTextFile("./test_stripe/audit_report.json", JSON.stringify(report, null, 2));
