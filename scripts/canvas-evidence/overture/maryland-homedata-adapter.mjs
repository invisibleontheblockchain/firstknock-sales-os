...
export function applyMarylandHomeDataAdapter({ rows, polygon, sourceTile, normalizedTiles, datasetVersion, observedAt, expectedSourceHash } = {}) {
  ...
  const countClassifications = (items) => items.reduce((out, property) => { out[property.canvass_eligibility] += 1; return out; }, { eligible: 0, excluded: 0, review: 0 });
  const beforeCounts = countClassifications(properties);
  const counts = countClassifications(normalizedTile.properties);
  const finalById = new Map(normalizedTile.properties.map((property) => [property.fk_property_id, property]));
  const transitionMatrix = Object.fromEntries(['eligible', 'excluded', 'review'].flatMap((from) => ['eligible', 'excluded', 'review'].map((to) => [`${from}_to_${to}`, 0])));
  properties.forEach((property) => { transitionMatrix[`${property.canvass_eligibility}_to_${finalById.get(property.fk_property_id).canvass_eligibility}`] += 1; });
  return { source_tile: augmentedSourceTile, normalized_tile: normalizedTile, evidence_ledger: ledger, source: { source_id: SOURCE_ID, provider: 'Maryland SDAT/MDP HomeData', dataset_version: datasetVersion, license: LICENSE, captured_at: observedAt, source_hash: sourceHash, mapping_version: MARYLAND_HOMEDATA_MAPPING_VERSION }, report: { source_hash: sourceHash, mapping_version: MARYLAND_HOMEDATA_MAPPING_VERSION, selected_record_count: areaRows.length, matched_record_count: matches.filter((match) => match.fk_property_id).length, accepted_assertion_count: assertions.length, high_confidence_assertion_count: ledger.filter((item) => item.accepted && item.match_confidence === 'high').length, conflict_downgrade_assertion_count: ledger.filter((item) => item.accepted && item.reason_codes.includes('homedata_conflict_downgrade_only')).length, before_homedata: { ...beforeCounts, review_percent: Number((beforeCounts.review / properties.length * 100).toFixed(1)) }, after_homedata: { ...counts, review_percent: Number((counts.review / properties.length * 100).toFixed(1)) }, transition_matrix: transitionMatrix, opposite_decision_flips: transitionMatrix.eligible_to_excluded + transitionMatrix.excluded_to_eligible, property_classification_counts: counts, review_percent: Number((counts.review / properties.length * 100).toFixed(1)), policy: { automatic_promotion_requires_high_confidence: true, conflicts_remain_review: true, ambiguous_matches_remain_review: true, property_type_separate_from_canvass_eligibility: true, access_separate_from_residential_use: true }, workload_authority: 'eligible_properties', batchdata_call_count: 0, rentcast_call_count: 0 } };
}