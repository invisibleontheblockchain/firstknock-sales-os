...
const DEFAULT_ADDITIONAL_VIEWERS = new Set(['christian@nativapest.com']);
...
function canViewPlatformCommandCenter(user: any) {
    ...
    const configured = String(Deno.env.get('PLATFORM_DASHBOARD_ALLOWED_EMAILS') || '')
        .split(',')
        .map(normalized)
        .filter(Boolean);
    const allowedEmails = new Set([...DEFAULT_ADDITIONAL_VIEWERS, ...configured]);
    return allowedEmails.has(normalized(user.email));
}
...
function isPrecisionDoorOutcome(log: any) {
    return PRECISION_DOOR_OUTCOMES.has(normalized(log?.parsed_status)) && !isPrecisionPhotoAudit(log);
}
...
function buildIdentityMaps(users: any[], members: any[]) {
    ...
    return { userById, userByEmail, memberById, memberByEmail, memberByManagerEmail };
}

function precisionIdentity(log: any, maps: any) {
    const email = normalized(log?.created_by);
    const user = maps.userByEmail.get(email);
    const member = maps.memberByManagerEmail.get(`${String(log?.manager_id || '')}:${email}`) || maps.memberByEmail.get(email);
...
function teamMemberIdentity(member: any, maps: any) {
    const user = maps.userById.get(String(member?.user_id || ''));
    const email = normalized(user?.email || member?.email);
...
function buildAdoptionWindowDays(now: number, windowDays: number) {
    const start = new Date(now);
    start.setUTCHours(0, 0, 0, 0);
    const days: any[] = [];
...
function buildAdoptionActivity(
    ...
    const days = buildAdoptionWindowDays(now, windowDays);
    const windowStart = timestamp(days[0]?.date) ?? now - windowDays * DAY_MS;
    const reps = new Map<string, any>();
    ...
    const touch = (identity: any, date: string, occurredAt: number | null, isSale: boolean) => {
        const rep = ensureRep(identity);
        const bucket = rep.days.get(date) || { logs: 0, sales: 0 };
        bucket.logs += 1;
        if (isSale) bucket.sales += 1;
    ...
    for (const log of interactionLogs) {
        const occurredAt = timestamp(log?.created_date || log?.updated_date);
        if (occurredAt === null || occurredAt < windowStart) continue;
        const identity = precisionIdentity(log, maps);
        touch(identity, dayKey(occurredAt), occurredAt, normalized(log?.parsed_status) === 'sold');
    }
    ...
    for (const event of canvasEvents) {
        ...
        if (occurredAt === null || occurredAt < windowStart) continue;
        const identity = canvasIdentity(event, maps);
        touch(identity, dayKey(occurredAt), occurredAt, normalized(event?.outcome) === 'sale');
    }
...
function buildStripeAnalytics(stripeData: any, users: any[], trend: any[], betaGrants: any[] = []) {
    ...
    const rows = subscriptions.map((subscription: any) => {
        ...
        const hasPaidHistory = paidInvoices.some((invoice: any) => invoiceBelongsToSubscription(invoice, subscription));
        const currentPeriodPaid = paidInvoices.some((invoice: any) => invoiceCoversSubscriptionPeriod(invoice, subscription));
