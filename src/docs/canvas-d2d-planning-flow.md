...
The manager creates the first preview explicitly. After that, changing the subdivision count debounces for 600 ms and refreshes the colored map preview automatically. A newer count cancels any obsolete calculation, and failed previews do not retry until the manager changes an input. Planning runs in a module worker so a large calculation does not freeze the map. The algorithm guard supports up to 20,000 work units, 50,000 segments, and a work-unit × territory budget of 2,000,000. The preview also measures the actual normalized UTF-8 draft against the 8 MB save limit and reserves 200 KB for later assignments and lifecycle metadata, so unusually detailed street data is rejected before the manager reviews an unsavable plan. Large accepted plans render authoritative colored street segments without generating legacy corridor-fill polygons; saving and server verification preserve the same street-ownership contract.
