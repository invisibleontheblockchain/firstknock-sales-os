...
export default function ZipCodeExplorer() {
  ...
  const handleCombineAllRoutes = () => {
    ...
    generatedRoutes.forEach(r => {
      combinedProps = [...combinedProps, ...r.properties];
    });

    // Recalculate total distance loosely
    let totalDist = 0;
    generatedRoutes.forEach(r => totalDist += (r.totalDistance || 0));
    ...
    toast.success("Successfully combined all generated routes into one.");
  };

  const handleDeleteAllRoutes = () => {
    setGeneratedRoutes([]);
    setActiveRoute(null);
    toast.info("All currently generated routes deleted.");
  };
            ...
            {generatedRoutes.length > 0 && (
              <>
                ...
                {generatedRoutes.length > 1 && (
                  <Button
                    variant="outline"
                    onClick={handleCombineAllRoutes}
