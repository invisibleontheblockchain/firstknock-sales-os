        ...
        <div className="flex flex-col h-screen bg-slate-900 text-slate-100 font-sans overflow-hidden">
            ...
            <nav className="bg-slate-800 border-t border-slate-700 z-20 safe-area-bottom">
                <div className="flex justify-around items-center h-16 max-w-md mx-auto">
                    <NavItem icon={Map} label="Map" to={createPageUrl('Home')} active />
                    <NavItem icon={List} label="List" to="#" />
                    <NavItem icon={Upload} label="Sync" to="#" />
...
function NavItem({ icon: Icon, label, to, active }) {
    return (
        <Link to={to} className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${active ? 'text-indigo-400' : 'text-slate-500 hover:text-slate-300'}`}>
