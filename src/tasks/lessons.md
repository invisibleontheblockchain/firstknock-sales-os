...
- When writing diagnostics that compare database date values, normalize with `new Date(value).toISOString().slice(0, 10)` before comparing; `String(date).slice(0, 10)` can produce weekday text like `Wed Jun 24` and create false positive date counts.
- For paid BatchData owner-change pulls, avoid duplicating provider-side sale qualification with multiple local hard gates. Keep local hard checks to coordinates, drawn-area containment, and clearly non-residential records; let exact-job BatchData rows pass repeated date/confidence/SFR route filters during verification pulls.
- When BatchData returns zero for a recent-sale window after local gates are loosened, build a provider escalation packet before changing logic again: identify the exact FetchJob, include the exact payload, run exact-date vs no-date probes, and explicitly separate provider raw zero-results from app-side filtering.
