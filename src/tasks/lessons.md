...
- Route generation has multiple date gates: provider request, exact-job candidate query, and final local route filter. Provider/candidate acquisition may use a broader safety window for deed lag, but the final visible route/display filter must honor the user's exact selected range.
