...
import {
  getCanvasPropertyClassificationSummary,
...
import CanvasPropertyExceptionSummary from '@/components/canvas/CanvasPropertyExceptionSummary';
...
function OpportunityMetric({ summary, confidencePercent, propertyAware = false }) {
  const range = formatCanvasOpportunityRange(summary);
  return <div className="rounded-xl border border-white/10 bg-black/30 p-2.5">
    <p className="text-[9px] font-bold uppercase text-gray-500">{propertyAware ? 'Eligible doors' : 'Estimated homes'}</p>
...
function PlanEvidenceReport({ workload, uncertainCount, releaseId, recommendation, onUseRecommendation }) {
  ...
  const rows = [
    ...
    ['Needs review', `${uncertainCount} ${uncertainCount === 1 ? 'property' : 'properties'}`],
...
function roadTruth(status, error) {
  if (status === 'ready') return { tone: 'good', title: 'Property evidence ready', detail: 'Canvas found classified properties linked by a verified street network. Changing the area count now reuses this snapshot.' };
  ...
  if (status === 'empty') return { tone: 'bad', title: 'No eligible properties found', detail: error?.message || 'Redraw around a neighborhood or review property evidence coverage.' };
...
function PlanSummary({ plan, boundaryAreaSqMiles, staleReason, residentialAnalysis }) {
  ...
  return <div className="space-y-2 rounded-2xl border border-white/10 bg-white/[0.03] p-3"><div className="grid grid-cols-2 gap-2"><Metric label="Global boundary" value={formatCanvasArea(boundaryAreaSqMiles)} /><Metric label="Areas" value={zones.length} /><Metric label="Eligible doors" value={Math.round(totalOpportunities).toLocaleString()} /><Metric label="Per-area range" value={range.replace('homes', 'doors')} /></div><div className={`rounded-xl border p-3 ${safetyReady ? 'border-green-400/20 bg-green-500/[0.08] text-green-100' : 'border-amber-400/20 bg-amber-500/[0.08] text-amber-100'}`}><p className="text-xs font-black">{safetyReady ? 'Territory checks passed' : staleReason || 'Territory checks need review'}</p><p className="mt-1 text-[10px] opacity-75">Eligible-property workload · connected streets · protected cul-de-sacs {deviation.verified ? `· max workload deviation ${Math.round(deviation.value)}%` : ''}</p></div>{qa.warnings?.length > 0 && <details className="rounded-xl border border-white/10 bg-black/20"><summary className="cursor-pointer px-3 py-2 text-[10px] font-black text-gray-400">Planning notes ({qa.warnings.length})</summary><div className="space-y-1 border-t border-white/10 p-3">{qa.warnings.map((warning, index) => <p key={`${String(warning)}:${index}`} className="text-[10px] leading-relaxed text-amber-100">• {typeof warning === 'string' ? warning : warning?.message || warning?.code}</p>)}</div></details>}</div>;
}

// Lock holds an area's exact streets through the next regeneration. Merge folds
// it into a neighbour — Canvas refuses areas that do not touch, because a
// territory a rep cannot walk end to end is not a territory.
function AreaEditControls({ zone, zones, disabled, locked, onToggleLock, onMergeInto }) {
  const others = zones.filter((candidate) => candidate.zone_number !== zone.zone_number);
  return <div className="mt-3 space-y-2 border-t border-white/10 pt-2.5">
    <button type="button" role="switch" aria-checked={locked} disabled={disabled} onClick={() => onToggleLock(zone)} className={`flex w-full items-center justify-between gap-2 rounded-lg border px-3 py-2 text-[10px] font-bold disabled:opacity-50 ${locked ? 'border-green-400/30 bg-green-500/15 text-green-100' : 'border-white/10 bg-black/25 text-gray-300'}`}>
      <span>{locked ? 'Locked — keeps these streets' : 'Lock this area'}</span>
      {locked ? <ShieldCheck className="h-3.5 w-3.5" /> : <Pencil className="h-3.5 w-3.5" />}
    </button>
    {others.length > 0 && onMergeInto && <Select disabled={disabled} value="" onValueChange={(value) => onMergeInto(zone, zones.find((candidate) => String(candidate.zone_id) === value))}>
      <SelectTrigger className="h-9 border-white/10 bg-black/30 text-[10px] text-gray-300"><SelectValue placeholder="Merge into a touching area..." /></SelectTrigger>
      <SelectContent className="z-[4000] border-white/10 bg-black text-white">{others.map((candidate) => <SelectItem key={candidate.zone_id} value={String(candidate.zone_id)}>Merge into Area {candidate.zone_number}</SelectItem>)}</SelectContent>
    </Select>}
  </div>;
}

function AreaPreviewList({ zones, selectedZoneNumber, setSelectedZoneNumber, compact, residentialAnalysis, showAssignees = false, membersById = new Map(), activeTeamMembers = [], disabled = false, onAssignment, lockedZoneIds = null, onToggleLock, onMergeInto }) {
...
export default function CanvasPlannerWorkspace(props) {
  ...
  const propertySummary = getCanvasPropertyClassificationSummary(residentialAnalysis);
  ...
  return <div className="relative flex h-full flex-col text-white">
    <div className="shrink-0 border-b border-white/10 p-4"><div className="flex items-start justify-between gap-3"><div><h2 className="flex items-center gap-2 font-extrabold tracking-wide text-purple-300"><MapIcon className="h-5 w-5" /> CANVAS</h2><p className="mt-1 text-[10px] text-gray-500">Balance eligible properties into connected areas. Assign people when ready.</p></div><button type="button" disabled={mutationsLocked && !deployed} onClick={onClose} className="rounded-full p-2 hover:bg-white/10 disabled:opacity-40" aria-label="Close Canvas"><X className="h-5 w-5 text-gray-300" /></button></div><div className="mt-3 grid grid-cols-2 gap-1 rounded-xl border border-white/10 bg-black/30 p-1"><button type="button" aria-pressed={buildView} onClick={() => { if (!buildView && plan) startAnotherArea(); else changeWorkspaceView('new_area'); }} className={`rounded-lg px-3 py-2 text-[10px] font-black ${buildView ? 'bg-purple-500 text-white' : 'text-gray-400 hover:text-white'}`}>NEW AREA</button><button type="button" aria-pressed={!buildView} onClick={() => changeWorkspaceView('areas')} className={`rounded-lg px-3 py-2 text-[10px] font-black ${!buildView ? 'bg-purple-500 text-white' : 'text-gray-400 hover:text-white'}`}>AREAS & ASSIGNMENTS</button></div></div>
