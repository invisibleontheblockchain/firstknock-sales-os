...
import { MAX_CANVAS_CLASSIFICATION_GROUP_UNITS } from '@/components/canvas/canvasProductionClient';
import {
...
function analysisTruth(status, source, error) {
  if (status === 'ready' && source === 'server') return { tone: 'good', title: 'Residential analysis ready', detail: 'Server evidence is pinned for this boundary. The map separates likely frontage, context, exclusions, and streets needing review.' };
  if (status === 'ready' && source === 'server_development') return { tone: 'idle', title: 'Development evidence only', detail: 'The server used the explicitly enabled public OSM fallback. Preview and draft review work, but production activation stays blocked.' };
  if (status === 'ready') return { tone: 'good', title: 'Development fallback preview ready', detail: `Public browser OSM evidence is being used only for local development while server evidence is unavailable.${error ? ` ${error}` : ''}` };
  if (status === 'loading') return { tone: 'loading', title: 'Analyzing residential work', detail: 'Canvas is reading opportunity and access evidence without hiding the boundary.' };
  if (status === 'unavailable') return { tone: 'bad', title: 'Residential analysis unavailable', detail: error || 'Street data loaded, but residential evidence could not be classified.' };
  return { tone: 'idle', title: 'Residential analysis follows street loading', detail: 'Draw one boundary to start the analysis.' };
}

function formatOpportunityRange(summary) {
  if (!summary || !Number.isFinite(Number(summary.expected))) return 'Not available';
  const low = Math.round(Number(summary.low) || 0).toLocaleString();
  const expected = Math.round(Number(summary.expected) || 0).toLocaleString();
  const high = Math.round(Number(summary.high) || 0).toLocaleString();
  return `${expected} expected (${low}-${high})`;
}

function ResidentialAnalysisSummary({ analysis, status, source, error }) {
  const truth = analysisTruth(status, source, error);
  const counts = getCanvasResidentialRoleCounts(analysis);
  const opportunity = getCanvasResidentialOpportunitySummary(analysis);
  const total = Object.values(counts).reduce((sum, count) => sum + Number(count || 0), 0);
  const pinnedServerEvidence = source === 'server';
  return <div className="space-y-2"><TruthRow icon={Network} {...truth} />{analysis && <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-black text-white">What the map found</p><p className="mt-1 text-[10px] leading-relaxed text-gray-500">Street colors explain the evidence before area colors show ownership.</p></div><span className={`shrink-0 rounded-full border px-2 py-1 text-[9px] font-black uppercase ${pinnedServerEvidence ? 'border-green-400/20 bg-green-500/10 text-green-200' : 'border-amber-400/20 bg-amber-500/10 text-amber-200'}`}>{source === 'server' ? 'Server evidence' : source === 'server_development' ? 'Server development fallback' : 'Browser development fallback'}</span></div><div className="mt-3 grid grid-cols-2 gap-2"><Metric label="Street units reviewed" value={total.toLocaleString()} /><Metric label="Residential workload" value={formatOpportunityRange(opportunity)} /></div><div className="mt-3 grid gap-1.5">{Object.entries(CANVAS_RESIDENTIAL_ROLE_META).map(([role, meta]) => <div key={role} className="flex items-center justify-between rounded-lg border border-white/[0.06] bg-black/20 px-2.5 py-2"><span className="flex min-w-0 items-center gap-2"><span className="h-2.5 w-2.5 shrink-0 rounded-full" style={{ background: meta.color }} /><span className="truncate text-[10px] font-bold text-gray-300">{meta.label}</span></span><strong className="text-[11px] text-white">{Number(counts[role] || 0).toLocaleString()}</strong></div>)}</div><p className="mt-3 rounded-lg border border-amber-400/15 bg-amber-500/[0.06] px-2.5 py-2 text-[10px] leading-relaxed text-amber-100">Empty-field or unmapped streets stay amber for review. Commercial or open-land context is excluded only when affirmative evidence exists.</p><p className="mt-2 text-[9px] text-gray-500">© OpenStreetMap contributors</p></div>}</div>;
}

function LargeAnalysisProgress({ progress, onCancel, cancelling }) {
  const completed = Math.max(0, Number(progress?.completed) || 0);
  const total = Math.max(0, Number(progress?.total) || 0);
  if (total <= 1) return null;
  const reportedPercent = Number(progress?.progressPct);
  const percent = Math.max(0, Math.min(100, Number.isFinite(reportedPercent)
    ? reportedPercent
    : completed / total * 100));
  const finalizing = progress?.status === 'finalizing';
  return <div className="rounded-xl border border-blue-400/20 bg-blue-500/[0.07] p-3"><div className="flex items-center justify-between text-[10px] font-black text-blue-100"><span>{finalizing ? 'Finalizing residential evidence' : 'Large-area residential analysis'}</span><span>{completed}/{total} tiles · {Math.round(percent)}%</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-black/30"><div className="h-full rounded-full bg-blue-400 transition-[width]" style={{ width: `${percent}%` }} /></div><p className="mt-2 text-[10px] leading-relaxed text-blue-100/70">Every tile must finish. Canvas will not preview, save, or send a partial street analysis. You may close Canvas and this job will resume here when you return.</p>{onCancel && <Button type="button" disabled={cancelling || finalizing} onClick={() => onCancel()} className="mt-2 h-8 w-full border border-red-300/20 bg-red-500/10 text-[10px] text-red-100 hover:bg-red-500/20 disabled:opacity-50">{cancelling ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <X className="h-3.5 w-3.5" />} Cancel analysis</Button>}</div>;
}
...
function PlanSummary({ plan, boundaryAreaSqMiles, staleReason, residentialAnalysis }) {
  ...
  const zoneOpportunities = zones.map((zone) => getCanvasZoneResidentialOpportunity(zone, residentialAnalysis)).filter(Boolean);
  ...
  const streetRange = streetMiles.length ? `${Math.min(...streetMiles).toFixed(1)}–${Math.max(...streetMiles).toFixed(1)} mi` : 'Unavailable';
  const opportunityRange = zoneOpportunities.length
    ? `${Math.round(Math.min(...zoneOpportunities.map((value) => value.expected))).toLocaleString()}–${Math.round(Math.max(...zoneOpportunities.map((value) => value.expected))).toLocaleString()} homes`
    : streetRange;
  return <div className="space-y-2 rounded-2xl border border-white/10 bg-white/[0.03] p-3"><div className="grid grid-cols-2 gap-2"><Metric label="Global boundary" value={formatCanvasArea(boundaryAreaSqMiles)} /><Metric label="Areas" value={zones.length} /><Metric label="Eligible streets" value={formatCanvasDistance(totalStreetMeters)} /><Metric label="Residential opportunity" value={formatOpportunityRange(opportunitySummary)} /><Metric label="Area workload range" value={opportunityRange} /><Metric label="Assignment" value="Later" /></div><div className={`rounded-xl border p-3 ${safetyReady ? 'border-green-400/20 bg-green-500/[0.08] text-green-100' : 'border-amber-400/20 bg-amber-500/[0.08] text-amber-100'}`}><p className="text-xs font-black">{safetyReady ? 'Street-safety checks passed' : staleReason || 'Street-safety checks need review'}</p><p className="mt-1 text-[10px] opacity-75">Connected areas · exclusive ownership · protected cul-de-sacs {deviation.verified ? `· max workload deviation ${Math.round(deviation.value)}%` : ''}</p></div>{qa.warnings?.length > 0 && <details className="rounded-xl border border-white/10 bg-black/20"><summary className="cursor-pointer px-3 py-2 text-[10px] font-black text-gray-400">Planning notes ({qa.warnings.length})</summary><div className="space-y-1 border-t border-white/10 p-3">{qa.warnings.map((warning, index) => <p key={`${String(warning)}:${index}`} className="text-[10px] leading-relaxed text-amber-100">• {typeof warning === 'string' ? warning : warning?.message || warning?.code}</p>)}</div></details>}</div>;
}
...
function AreaPreviewList({ zones, selectedZoneNumber, setSelectedZoneNumber, compact, residentialAnalysis, showAssignees = false, membersById = new Map(), activeTeamMembers = [], disabled = false, onAssignment }) {
  ...
  return <div className={`grid gap-2 overflow-y-auto pr-1 ${compact ? 'max-h-52' : 'max-h-64'}`}>{zones.map((zone, index) => {
    ...
    const opportunity = getCanvasZoneResidentialOpportunity(zone, residentialAnalysis);
    const workload = opportunity
      ? `${Math.round(opportunity.expected).toLocaleString()} expected homes (${Math.round(opportunity.low).toLocaleString()}–${Math.round(opportunity.high).toLocaleString()})`
      : `${formatCanvasDistance(streetMeters)} · ${share.toFixed(1)}% of street workload`;
...
function AmberResolutionPanel({ analysis, source, applying, error, onApply }) {
  ...
  useEffect(() => {
    ...
    setReason('');
    setOpportunityCount('');
    setApplyNamedGroup(false);
  }, [selectedUnitId]);

  if (!uncertainUnits.length) {
    return <section className="rounded-2xl border border-green-400/20 bg-green-500/[0.07] p-3"><p className="flex items-center gap-2 text-xs font-black text-green-100"><CheckCircle2 className="h-4 w-4" /> No amber streets remain</p><p className="mt-1 text-[10px] leading-relaxed text-green-100/70">Residential evidence no longer blocks activation. Assignment can continue below.</p><p className="mt-2 text-[9px] text-gray-500">© OpenStreetMap contributors</p></section>;
  }

  const selectedUnit = uncertainUnits.find((unit) => String(unit?.id || unit?.street_unit_id || '') === selectedUnitId);
  const selectedStreetNames = (Array.isArray(selectedUnit?.streetNames || selectedUnit?.street_names)
    ? selectedUnit?.streetNames || selectedUnit?.street_names
    : [selectedUnit?.streetNames || selectedUnit?.street_names])
    .map((name) => String(name || '').trim()).filter(Boolean);
  const primaryStreetName = selectedStreetNames[0] || '';
  const namedGroupUnits = primaryStreetName ? uncertainUnits.filter((unit) => {
    const names = Array.isArray(unit?.streetNames || unit?.street_names)
      ? unit?.streetNames || unit?.street_names
      : [unit?.streetNames || unit?.street_names];
    return names.some((name) => String(name || '').trim().toLowerCase() === primaryStreetName.toLowerCase());
  }) : [];
  const groupChoiceTooLarge = ['transit', 'exclude'].includes(choice)
    && namedGroupUnits.length > MAX_CANVAS_CLASSIFICATION_GROUP_UNITS;
  const groupChoiceAllowed = ['transit', 'exclude'].includes(choice)
    && namedGroupUnits.length > 1
    && !groupChoiceTooLarge;
  const submit = async () => {
    const streetUnitIds = applyNamedGroup && groupChoiceAllowed
      ? namedGroupUnits.map((unit) => String(unit?.id || unit?.street_unit_id || '')).filter(Boolean)
      : [selectedUnitId];
    const saved = await onApply?.({ streetUnitId: selectedUnitId, streetUnitIds, choice, reason, opportunityCount });
    if (saved) {
      setChoice('');
      setReason('');
      setOpportunityCount('');
  ...
  const residentialCountValid = choice !== 'residential'
    || (Number.isSafeInteger(Number(opportunityCount)) && Number(opportunityCount) > 0);
  return <section className="space-y-3 rounded-2xl border border-amber-400/25 bg-amber-500/[0.07] p-3"><div><p className="flex items-center gap-2 text-xs font-black text-amber-100"><AlertTriangle className="h-4 w-4" /> Resolve amber streets ({uncertainUnits.length})</p><p className="mt-1 text-[10px] leading-relaxed text-gray-300">Amber contributes zero workload. Preview and save remain available, but sending stays blocked until every amber street has a documented decision.</p></div><div><label className="text-[10px] font-bold uppercase text-gray-400">Street or pocket</label><Select value={selectedUnitId} onValueChange={selectUnit}><SelectTrigger className="mt-1 h-10 border-white/10 bg-black/30 text-xs text-white"><SelectValue placeholder="Choose an amber street" /></SelectTrigger><SelectContent className="z-[4000] border-white/10 bg-black text-white">{uncertainUnits.map((unit, index) => { const id = String(unit?.id || unit?.street_unit_id || ''); const names = unit?.streetNames || unit?.street_names || []; const label = Array.isArray(names) && names.filter(Boolean).length ? names.filter(Boolean).join(' / ') : `Unmapped street ${index + 1}`; return <SelectItem key={id} value={id}>{label}</SelectItem>; })}</SelectContent></Select></div>{selectedUnit && <p className="rounded-lg border border-white/10 bg-black/20 px-2.5 py-2 font-mono text-[9px] text-gray-500">{selectedUnitId}</p>}<div><p className="text-[10px] font-bold uppercase text-gray-400">Decision</p><div className="mt-1 grid grid-cols-3 gap-2">{[['residential', 'Residential'], ['transit', 'Transit'], ['exclude', 'Exclude']].map(([value, label]) => <button key={value} type="button" aria-pressed={choice === value} onClick={() => { setChoice(value); if (value === 'residential') setApplyNamedGroup(false); }} className={`rounded-lg border px-2 py-2 text-[10px] font-black ${choice === value ? 'border-amber-300 bg-amber-400/20 text-amber-50' : 'border-white/10 bg-black/20 text-gray-300'}`}>{label}</button>)}</div></div>{choice === 'residential' && <div><label className="text-[10px] font-bold uppercase text-gray-400">Known homes · required</label><Input type="number" min={1} step={1} value={opportunityCount} onChange={(event) => setOpportunityCount(event.target.value)} placeholder="Enter a positive whole-number count" className="mt-1 h-10 border-white/10 bg-black/30 text-xs text-white" /></div>}{groupChoiceAllowed && <button type="button" role="checkbox" aria-checked={applyNamedGroup} onClick={() => setApplyNamedGroup((current) => !current)} className={`flex w-full items-start gap-2 rounded-xl border p-3 text-left text-[10px] ${applyNamedGroup ? 'border-amber-300/60 bg-amber-400/15 text-amber-50' : 'border-white/10 bg-black/20 text-gray-300'}`}><span className={`mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded border ${applyNamedGroup ? 'border-amber-200 bg-amber-400 text-black' : 'border-white/20'}`}>{applyNamedGroup && <CheckCircle2 className="h-3 w-3" />}</span><span>Apply this audited {choice === 'exclude' ? 'exclusion' : 'transit'} decision to all {namedGroupUnits.length} amber units named {primaryStreetName} as one atomic revision. Residential overrides always remain one-at-a-time because each needs its own home count.</span></button>}{groupChoiceTooLarge && <p className="rounded-lg border border-amber-400/20 bg-black/20 p-2.5 text-[10px] leading-relaxed text-amber-100">{primaryStreetName} has {namedGroupUnits.length} amber units. Atomic audited groups are limited to {MAX_CANVAS_CLASSIFICATION_GROUP_UNITS}; review a smaller group or individual units.</p>}<div><label className="text-[10px] font-bold uppercase text-gray-400">Reason · required</label><Input maxLength={500} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="What evidence supports this decision?" className="mt-1 h-10 border-white/10 bg-black/30 text-xs text-white" /></div>{!serverReady && <p className="rounded-lg border border-red-400/20 bg-red-500/[0.08] p-2.5 text-[10px] leading-relaxed text-red-100">Server evidence is unavailable. This control fails closed: the amber decision cannot be dismissed locally, and activation remains blocked.</p>}{error && <p role="alert" className="text-[10px] leading-relaxed text-red-200">{error}</p>}<Button type="button" disabled={applying || !serverReady || !selectedUnitId || !choice || !residentialCountValid || !reason.trim()} onClick={submit} className="h-10 w-full bg-amber-500 font-black text-black hover:bg-amber-400 disabled:bg-gray-800 disabled:text-gray-500">{applying ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} {applyNamedGroup && groupChoiceAllowed ? `Save ${namedGroupUnits.length} units atomically` : 'Save classification revision'}</Button><p className="text-[9px] text-gray-500">© OpenStreetMap contributors</p></section>;
}
...
export default function CanvasPlannerWorkspace(props) {
  const {
    ...
    residentialAnalysis,
    residentialAnalysisStatus,
    residentialAnalysisSource,
    residentialAnalysisError,
    unresolvedAmberCount,
    amberActivationBlocked,
    applyingClassificationOverride,
    classificationOverrideError,
    applyClassificationRevision,
    generationBlockers,
    generatePlan,
    generating,
    plan,
