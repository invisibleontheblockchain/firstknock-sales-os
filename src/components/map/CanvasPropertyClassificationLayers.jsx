...
export default function CanvasPropertyClassificationLayers({ analysis }) {
  ...
  return <LayerGroup>{properties.map((property) => {
    ...
    return <CircleMarker key={property.property_id} center={[property.point.lat, property.point.lng]} radius={state === 'review' ? 6 : 4} pathOptions={{ color: '#FFFFFF', weight: state === 'review' ? 2 : 1, fillColor: COLORS[state] || COLORS.review, fillOpacity: state === 'excluded' ? 0.45 : 0.9 }}><Tooltip><strong>{state === 'eligible' ? 'Knockable residential' : state === 'excluded' ? 'Excluded' : 'Needs review'}</strong><br />{property.property_type?.replaceAll('_', ' ')} · {property.confidence_percent}% confidence{property.classification_reasons?.[0] ? <><br />{property.classification_reasons[0].replaceAll('_', ' ')}</> : null}</Tooltip></CircleMarker>;
  })}</LayerGroup>;
}