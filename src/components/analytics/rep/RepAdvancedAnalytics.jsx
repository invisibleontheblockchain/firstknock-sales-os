...
import { Activity, Repeat, Hash, CalendarDays, PhoneCall, Trophy } from 'lucide-react';
import {
...
export default function RepAdvancedAnalytics({ logs, filteredLogs, properties, appointments, dateDays, selectedDate }) {
  const volumeTrend = useMemo(() => {
    const days = selectedDate ? 1 : Math.min(dateDays, 90);
    const anchorDate = selectedDate || new Date();
    return Array.from({ length: days }, (_, i) => {
      const day = subDays(anchorDate, days - 1 - i);
      const dayWindow = getAnalyticsDateWindow({ selectedDate: day });
      const dayLogs = filteredLogs.filter((log) => isWithinAnalyticsDateWindow(log.created_date, dayWindow));
      const contacts = dayLogs.filter((l) => !NON_CONTACT.includes(l.parsed_status)).length;
      return {
        date: format(day, selectedDate ? 'MMM d' : days <= 14 ? 'EEE' : 'M/d'),
        knocks: dayLogs.length,
        contacts,
        sales: dayLogs.filter((l) => SALES.includes(l.parsed_status)).length,
      };
    });
  }, [filteredLogs, dateDays, selectedDate]);

  const dayOfWeek = useMemo(() => {
    const days = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
    const buckets = days.map((d) => ({ day: d, knocks: 0, sales: 0 }));
    filteredLogs.forEach((log) => {
      const timestamp = parseAnalyticsTimestamp(log.created_date);
      if (!timestamp) return;
  ...
  const statusVelocity = useMemo(() => {
    ...
    const configured = Object.entries(map).map(([s, cfg]) => {
      const count = filteredLogs.filter((l) => l.parsed_status === s).length;
      return { ...cfg, status: s, count };
    });
    const otherCount = filteredLogs.filter((log) => !knownStatuses.has(log.parsed_status)).length;
    if (selectedDate && otherCount > 0) {
      configured.push({ label: 'Other', color: '#8b5cf6', status: 'OTHER', count: otherCount });
    }
    return configured.filter((s) => s.count > 0).sort((a, b) => b.count - a.count);
  }, [filteredLogs, selectedDate]);

  const maxStatus = statusVelocity[0]?.count || 1;

  const summaryStats = useMemo(() => {
    const uniqueDays = new Set(filteredLogs
      .map((log) => parseAnalyticsTimestamp(log.created_date))
      .filter(Boolean)
      .map((date) => format(date, 'yyyy-MM-dd')));
    const uniqueDoors = new Set(filteredLogs.map((l) => l.address_hash).filter(Boolean));
    if (selectedDate) {
      const contacts = filteredLogs.filter((log) => !NON_CONTACT.includes(log.parsed_status)).length;
      const wins = filteredLogs.filter((log) => SALES.includes(log.parsed_status)).length;
      return [
        { label: 'Knocks', value: filteredLogs.length, icon: Activity, color: '#6366f1' },
        { label: 'Contacts', value: contacts, icon: PhoneCall, color: '#06b6d4' },
        { label: 'Wins', value: wins, icon: Trophy, color: '#22c55e' },
  ...
  const visibleDayPerformance = selectedDate
    ? dayOfWeek.filter((day) => day.day === format(selectedDate, 'EEE'))
    : dayOfWeek;
          ...
          return (
            <div key={m.label} className="rounded-xl border border-white/[0.06] bg-[#111113] p-2.5 md:p-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-[8px] md:text-[9px] font-bold uppercase tracking-[0.1em] text-gray-400">{m.label}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-2 md:gap-3">
        ...
        <div className="rounded-xl border border-white/[0.06] bg-[#111113] p-3 md:p-4">
          <h3 className="text-xs md:text-sm font-black text-white mb-0.5">
            {selectedDate ? format(selectedDate, 'EEEE, MMM d, yyyy') : 'Day Performance'}
          </h3>
          <p className="text-[9px] md:text-[10px] text-gray-400 mb-2">
            {selectedDate ? 'Knocks and wins on the selected day' : 'Best days for results'}
          </p>
          <div className="space-y-1.5">
            {visibleDayPerformance.map((d) => (
              <div key={d.day} className="flex items-center gap-2">
                <span className="text-[10px] font-bold text-gray-400 w-7">{d.day}</span>
                <div className="flex-1 h-5 rounded-md bg-white/[0.03] overflow-hidden relative">
                  ...
                  <span className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[8px] font-bold text-gray-400">{d.knocks}</span>
                </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-[#111113] p-3 md:p-4">
          ...
          {statusVelocity.length > 0 ? (
            ...
            <div className="flex min-h-[96px] items-center justify-center rounded-lg border border-dashed border-white/[0.08] px-4 text-center">
              <p className="text-[10px] text-gray-400">No activity outcomes were recorded for this period.</p>
            </div>
          )}
        </div>
      </div>
