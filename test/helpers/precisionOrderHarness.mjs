// Precision order-control test harness.
//
// Ported from the PR #73 audit branch (hardening/precision-order-control-model-2
// @ 4ca3078f) where it was mutation-validated: 30 mutations of production code,
// 30 caught, 0 survived. Only the harness is carried over — the fixture corpus,
// vendored PR #66 source and audit archive deliberately stay on that branch.
//
...
export function loadSharedOrderSafety() {
  ...
  assert.ok(names.length > 0, 'the shared order-safety module exposes no exports');
  let collected = null;
  vm.runInNewContext(
