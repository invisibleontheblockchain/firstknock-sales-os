...
import { countVisitOutcomes, formatRunRouteAge } from '../src/components/logic/outcomeStatus.js';
...
test('visits count every saved door decision but exclude notes and workflow changes', () => {
    assert.equal(countVisitOutcomes([
        { parsed_status: 'NO_ANSWER' },
        { parsed_status: 'NO_ANSWER' },
        { parsed_status: 'SOLD' },
        { parsed_status: 'ELIGIBLE', counts_as_knock: false, workflow_action: 'CLEAR_TO_TODO' },
        { source: 'house_note', description: 'Gate code' },
    ]), 3);
});
...
test('Run Route keeps every stop visible with explicit outcome presentation', () => {
    ...
    assert.match(checklist, /scheduleRouteStopAtTop/);
    assert.match(checklist, /<VisitBadge count=\{visitCount\} \/>/);
    assert.doesNotMatch(checklist, />\s*Return\s*</);
    assert.match(propertyCard, /visitCount = 0/);
    assert.match(repHome, /visitCountsByAddress/);
