...
let splitRouteCode;
let splitRouteCreationMatchesPlan;
let routeMembershipMatches;
...
test('child records use route names, real metrics, safe metadata, and pending dispatch state', () => {
  ...
  records.forEach((record, index) => {
    ...
    assert.equal(record.metadata.split_source.operation_id, 'split-operation-1');
...
test('source archival requires every persisted child manifest and a unique saved ID', () => {
  const homes = Array.from({ length: 12 }, (_, index) => property(index));
  const route = savedRoute(homes);
  const plan = buildOptimizedSplitPlan({ route, sizingMode: 'route_count', value: 3 });
  const records = buildSplitRouteRecords({ route, plan, managerId: 'manager-1' });
  const persisted = records.map((record, index) => ({ ...record, id: `child-${index}` })).reverse();

  assert.equal(splitRouteCreationMatchesPlan(records, persisted), true);
  assert.equal(splitRouteCreationMatchesPlan(records, persisted.slice(1)), false);
  assert.equal(splitRouteCreationMatchesPlan(records, persisted.map((child) => ({ ...child, id: 'duplicate' }))), false);
  assert.equal(splitRouteCreationMatchesPlan(records, persisted.map((child, index) => (
    index === 0 ? { ...child, property_hashes: child.property_hashes.slice(1) } : child
  ))), false);
});

test('source membership comparison detects added, removed, and duplicated homes', () => {
  assert.equal(routeMembershipMatches(['a', 'b', 'c'], ['c', 'a', 'b']), true);
  assert.equal(routeMembershipMatches(['a', 'b', 'c'], ['a', 'b', 'd']), false);
  assert.equal(routeMembershipMatches(['a', 'b'], ['a', 'b', 'c']), false);
  assert.equal(routeMembershipMatches(['a', 'a'], ['a', 'a']), false);
});

test('modal reconciles child and archive responses before destructive cleanup', () => {
  const modal = readFileSync(new URL('../src/components/routes/SplitRouteModal.jsx', import.meta.url), 'utf8');
  assert.match(modal, /operationId = createSplitOperationId\(\)/);
  assert.match(modal, /findCreatedRoutesForOperation\(operationId\)/);
  assert.match(modal, /sourceArchivedForOperation\(confirmedSource, operationId, createdRoutes\)/);
  assert.match(modal, /result\.value\?\.success === true/);
  assert.match(modal, /routeMembershipMatches\(plannedSourceHashes, currentSource\?\.property_hashes\)/);
  assert.match(modal, /SavedRoute\.updateMany\(/);
  assert.match(modal, /updated_date: currentSource\.updated_date/);
  assert.match(modal, /Number\(archiveMutation\?\.updated\) === 1/);
  assert.match(modal, /Number\(archiveMutation\?\.updated\) === 0/);
});
