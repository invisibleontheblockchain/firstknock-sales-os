...
test('platform admin receives complete empty app metrics while Stripe is unconfigured', async () => {
  const base44 = {
    auth: { me: async () => ({ id: 'admin_1', role: 'admin', app_role: 'rep' }) },
...
test('Christian is an explicit private HQ viewer without a platform-admin role', async () => {
  const base44 = {
    auth: { me: async () => ({ id: 'christian_1', role: 'owner', email: 'Christian@nativapest.com' }) },
    asServiceRole: emptyService(),
  };
  const { handler } = loadFunction({ base44 });
  const result = await invoke(handler);
...
test('dashboard polls every 15 seconds and independently guards the page', () => {
  ...
  assert.match(page, /adminDiagnostics', \{ view: 'platform_command_center' \}/);
...
test('HQ navigation exposes every section as one responsive accessible tab', () => {
  const page = readSource('src/admin/AdminDashboard.jsx');
  const styles = readSource('src/admin/admin.css');
  const expectedSections = ['field', 'revenue', 'pulse', 'cash', 'leaderboard', 'live', 'customers', 'operations'];
